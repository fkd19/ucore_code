...
alloc_proc(void) {
     ...
     */
    //LAB8:EXERCISE2 2019211103 HINT:need add some code to init fs in proc_struct, ...
    	proc->pid = -1;
		proc->mm = NULL;
		proc->cr3 = boot_cr3;
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    //LAB8:EXERCISE2 2019211103  HINT:how to copy the fs in parent's proc_struct?
    /*
...
load_icode(int fd, int argc, char **kargv) {
    /* LAB8:EXERCISE2 2019211103  HINT:how to load the file with handler fd  in to process's memory?
     * 								   how to setup argc/argv?
     * MACROs or Functions:
	...
	//(3.1) read raw data content in file and resolve elfhdr
	struct elfhdr elf;
	ret = load_icode_read(fd, &elf, sizeof(struct elfhdr), 0);//elf offset is 0
	if (ret){
		goto bad_elf_cleanup_pgdir;
	}
	//cprintf("finish loading elf!\n");
	//cprintf("elf.e_magic=%d, ELF_MAGIC=%d\n", elf.e_magic, ELF_MAGIC);
	...
	struct proghdr ph;
	struct Page *page;
	//cprintf("e_phnum=%d, e_phoff=%d\n", elf.e_phnum, elf.e_phoff);
	for (i=0; i<elf.e_phnum; i++){
		ret = load_icode_read(fd, &ph, sizeof(struct proghdr), elf.e_phoff + i * sizeof(struct proghdr));
		if (ret){
			//cprintf("ret=%d\n", ret);
			goto bad_cleanup_mmap;
		}
		//cprintf("finish loading proghdr %d\n", i);
		...
		while (start < end) {
			...
			//cprintf("load %d bytes, offset=%d, dst addr=%p, ret=%d\n",
			//		size, offset, page2kva(page)+off, ret);
			if (ret){
				goto bad_cleanup_mmap;
			}
	...
	uintptr_t ustacktop = USTACKTOP - ROUNDUP(argv_size, sizeof(long));
	char **uargv = (char **)(ustacktop - argc * sizeof(char *));

	argv_size = 0;
	for (i=0; i<argc; i++){
		uargv[i] = strcpy((char*)(ustacktop + argv_size), kargv[i]);
		argv_size += strnlen(kargv[i], EXEC_MAX_ARG_LEN+1)+1;
	}
	ustacktop = (uintptr_t)uargv - sizeof(int);
	*(int*)ustacktop = argc;
