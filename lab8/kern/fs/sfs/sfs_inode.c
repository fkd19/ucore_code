...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
    ...
    *alenp = 0;
    ...
    //LAB8:EXERCISE1 2019211103 HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc.
	...
	*/
    int i=0;
    //cprintf("start pos=%d, end pos = %d\n", offset, endpos);
    blkoff = offset % SFS_BLKSIZE;
    if (blkoff > 0){
    	size = (nblks != 0) ? (SFS_BLKSIZE - blkoff) : (endpos - offset);
    	ret = sfs_bmap_load_nolock(sfs, sin, blkno, &ino);
    	/*
    	if (sfs_buf_op == sfs_rbuf){
    		cprintf("read %d bytes, blkno=%d, disk_node=%d\n", size, blkno, ino);
    	}else{
    		cprintf("write %d bytes, blkno=%d, disk_node=%d\n", size, blkno, ino);
    	}
		*/
    	if (ret){
    		goto out;
    ...
    if (blkoff > 0){
		...
		*/
		...
		ret = sfs_buf_op(sfs, buf+alen, blkoff, ino, 0);
		if (ret){
			goto out;
		}
		alen += blkoff;
...
out:
    *alenp = alen;
    ...
    //cprintf("sfs_io_noblock() return %d\n", ret);
    return ret;
}
